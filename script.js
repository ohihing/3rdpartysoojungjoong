/**
 * [V4: 디테일 문구 추가] 교보문고 통합본 + 안내 텍스트 삽입
 */
...
function render() {
    ...
    if (currentPage === 'dashboard') {
        ...
        let configs = [];
        // [디테일 추가] 타이틀 옆에 안내 텍스트 삽입
        const recentLabel = '2. 최신간도서 Best 5 <span style="font-size:0.75rem; color:#999; font-weight:normal; margin-left:4px; letter-spacing:0;">(조회일 기준 출간 한달내 도서 대상)</span>';
        ...
        if (isIdx) {
            ...
            configs = [
                ...
                { t: recentLabel, k: `k_${k_key}_m`, type: 'qty', l: 5, sub: subTxt, data: superFresh },
            ...
            configs = [
                ...
                { t: recentLabel, k: `${s_prefix}_qty_m`, type: 'qty', l: 5, sub: subTxt, data: superFresh },
...
function getStat(val) {
    if (isNaN(val) || val === null) return { s: '-', c: 'val-no-change' };
